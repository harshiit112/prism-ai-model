...
.hero h1 span { color: #71717a; transition: all 0.5s ease; }
.hero h1:hover span { color: #ffffff; text-shadow: 0 0 20px rgba(255, 255, 255, 0.8); }
.hero-sub {
    font-size: 0.98rem;
    font-weight: 300;
    color: #8e8e93;
...
# Notify user if agent module failed to load
if not AGENTS_LOADED:
    st.error(f"⚠️ **Module Import Error:** Could not import `agents.py`. Ensure `agents.py` is located in your project root.\n\n`Details: {IMPORT_ERROR_MSG}`")
...
with col_input:
    ...
    run_btn = st.button("⚡  Run Research Pipeline", use_container_width=True, disabled=not AGENTS_LOADED)
    st.markdown('</div>', unsafe_allow_html=True)
...
if run_btn:
    if not topic.strip():
        st.warning("Please enter a research topic first.")
    elif not AGENTS_LOADED:
        st.error("Cannot run pipeline: Agent modules are missing or failed to import.")
    else:
        st.session_state.results = {}
        st.session_state.running = True
        st.session_state.done = False
...
if st.session_state.running and not st.session_state.done and AGENTS_LOADED:
    results = {}
    topic_val = st.session_state.topic_input
